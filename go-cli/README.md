# heat-stress-warning-cli

boru-driven command-line client **and** interactive REPL for the HeatStressWarning
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/heat-stress-warning-cli)
make build

# 2. See usage (words, entities, env vars)
./heat-stress-warning-cli --help

# 3. Provide credentials once, via the environment
export HEAT_STRESS_WARNING_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./heat-stress-warning-cli list heat_stress_warning_en
./heat-stress-warning-cli list heat_stress_warning_sc

# 5. Override the API base URL for a single call
HEAT_STRESS_WARNING_BASE=https://api.example.com ./heat-stress-warning-cli list heat_stress_warning_en

# 6. No arguments -> interactive REPL
./heat-stress-warning-cli
heat-stress-warning> list heat_stress_warning_en
heat-stress-warning> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/heat-stress-warning-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/heat-stress-warning-cli list heat_stress_warning_en
   ```
...
Run with no arguments to open a REPL (prompt `heat-stress-warning>`). Each line is
evaluated as its own boru expression:

```text
$ ./heat-stress-warning-cli
heat-stress-warning> list heat_stress_warning_en
heat-stress-warning> /help
heat-stress-warning> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `heat_stress_warning_en`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/heat-stress-warning-cli`. |
...
(`./heat-stress-warning-cli <expr>`) and interactively (the REPL), and expressions compose the
