...
// HeatStressWarning SDK pipeline test
//
...
{
    private static function utility(): HeatStressWarningUtility
    {
        return new HeatStressWarningUtility();
    }
    ...
    private static function ctx(array $over = []): HeatStressWarningContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new HeatStressWarningContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new HeatStressWarningOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof HeatStressWarningError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new HeatStressWarningOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = HeatStressWarningMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new HeatStressWarningOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = HeatStressWarningMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new HeatStressWarningOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = HeatStressWarningMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HeatStressWarningSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = HeatStressWarningMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new HeatStressWarningResponse([]),
            'result' => new HeatStressWarningResult([]),
        ]);
        [, $err] = HeatStressWarningMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec([]),
            'response' => null,
            'result' => new HeatStressWarningResult([]),
        ]);
        [, $err] = HeatStressWarningMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec(['step' => 's']),
            'response' => new HeatStressWarningResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new HeatStressWarningResult([]),
        ]);
        [, $err] = HeatStressWarningMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec(['step' => 's']),
            'response' => new HeatStressWarningResponse(self::resp(200, ['v' => 1])),
            'result' => new HeatStressWarningResult([]),
        ]);
        [, $err] = HeatStressWarningMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec(['step' => 's']),
            'response' => new HeatStressWarningResponse(self::resp(200, ['v' => 2])),
            'result' => new HeatStressWarningResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        HeatStressWarningMakeResponse::call($ctx);
    ...
    {
        $preset = new HeatStressWarningResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec([]),
            'response' => new HeatStressWarningResponse([]),
            'result' => new HeatStressWarningResult([]),
        ...
        [$got, $err] = HeatStressWarningMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new HeatStressWarningResult([])]);
        [, $err] = HeatStressWarningMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new HeatStressWarningSpec([]), 'result' => null]);
        [, $err] = HeatStressWarningMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HeatStressWarningSpec(['step' => 's']),
            'result' => new HeatStressWarningResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new HeatStressWarningOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HeatStressWarningMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new HeatStressWarningSpec(['step' => 's']),
            'result' => new HeatStressWarningResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new HeatStressWarningOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = HeatStressWarningMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new HeatStressWarningResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new HeatStressWarningSpec([]),
            'result' => new HeatStressWarningResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = HeatStressWarningMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HeatStressWarningMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new HeatStressWarningError('boom', 'boom');
        $utility->fetcher = function (HeatStressWarningContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HeatStressWarningSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = HeatStressWarningMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (HeatStressWarningContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = HeatStressWarningMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(HeatStressWarningResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        HeatStressWarningMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (HeatStressWarningContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new HeatStressWarningSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = HeatStressWarningMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new HeatStressWarningResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new HeatStressWarningSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = HeatStressWarningMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = HeatStressWarningMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = HeatStressWarningMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new HeatStressWarningResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, HeatStressWarningDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new HeatStressWarningResult(['ok' => false])]);
        $this->expectException(HeatStressWarningError::class);
        HeatStressWarningDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new HeatStressWarningResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', HeatStressWarningMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        HeatStressWarningMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new HeatStressWarningBaseFeature();
        $b = new HeatStressWarningBaseFeature();
        HeatStressWarningFeatureAdd::call($ctx, $a);
        HeatStressWarningFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): HeatStressWarningBaseFeature
    {
        $f = new HeatStressWarningBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        HeatStressWarningFeatureAdd::call($ctx, self::named_feature('a'));
        HeatStressWarningFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        HeatStressWarningFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        HeatStressWarningFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        HeatStressWarningFeatureAdd::call($ctx, $replace);
        ...
        HeatStressWarningFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): HeatStressWarningContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new HeatStressWarningSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = HeatStressWarningPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        HeatStressWarningPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new HeatStressWarningResponse(['headers' => null]),
            'result' => new HeatStressWarningResult([]),
        ]);
        HeatStressWarningResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        HeatStressWarningResultBody::call($ctx);
