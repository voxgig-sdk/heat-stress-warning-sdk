...
// HeatStressWarning SDK retry feature
...
class HeatStressWarningRetryFeature extends HeatStressWarningBaseFeature
{
    ...
    public function init(HeatStressWarningContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HeatStressWarningContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HeatStressWarningContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HeatStressWarningContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
