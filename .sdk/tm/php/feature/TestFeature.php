...
// HeatStressWarning SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HeatStressWarningTestFeature extends HeatStressWarningBaseFeature
{
    ...
    public function init(HeatStressWarningContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HeatStressWarningContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HeatStressWarningParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HeatStressWarningContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HeatStressWarningContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HeatStressWarningParam::call($ctx, $k);
