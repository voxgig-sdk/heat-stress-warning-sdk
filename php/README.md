# HeatStressWarning PHP SDK



The PHP SDK for the HeatStressWarning API — an entity-oriented client using PHP conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->HeatStressWarningEn()` — with named operations (`list`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($heatstresswarningens as $record) {
...
print_r(array_map(fn($item) => $item->data_get(), $heatstresswarningen));
```
...
```php
$client = HeatStressWarningSDK::test($testopts, $sdkopts);
```
...
```
php/
├── heatstresswarning_sdk.php          -- Main SDK class
...
The main class (`heatstresswarning_sdk.php`) exports the SDK class
