...
// HeatStressWarning SDK feature test
//
...
class FtCtrl extends HeatStressWarningControl
{
...
class FtClient
{
    ...
    private HeatStressWarningUtility $utility;

    public function __construct(HeatStressWarningUtility $utility, array $options)
    {
    ...
    public function get_utility(): HeatStressWarningUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HeatStressWarningContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HeatStressWarningContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HeatStressWarningContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HeatStressWarningConfig::make_feature($name);
    ...
    private static function buildUrl(HeatStressWarningSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HeatStressWarningError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HeatStressWarningResponse($fetched);
        }
    ...
    private function fail(HeatStressWarningContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HeatStressWarningContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
