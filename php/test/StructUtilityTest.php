...
// HeatStressWarning SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HeatStressWarningOmni::makeStructRunner(
    ...
    {
        return HeatStressWarningOmni::objmodel(HeatStressWarningOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HeatStressWarningOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HeatStressWarningOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HeatStressWarningOmni::structfix($now, Struct::undef()))
                        !== json_encode(HeatStressWarningOmni::structfix($val, Struct::undef()))) {
