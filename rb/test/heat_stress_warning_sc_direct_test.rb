...
class HeatStressWarningScDirectTest < Minitest::Test
  def test_direct_list_heat_stress_warning_sc
    setup = heat_stress_warning_sc_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-heat_stress_warning_sc", setup[:live] ? "live" : "unit")
    if _should_skip
...
def heat_stress_warning_sc_direct_setup(mockres)
