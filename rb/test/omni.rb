...
module HeatStressWarningOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if HeatStressWarningOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = HeatStressWarningOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| HeatStressWarningOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || HeatStressWarningOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = HeatStressWarningSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = HeatStressWarningError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { HeatStressWarningOmni.errify(err) }
