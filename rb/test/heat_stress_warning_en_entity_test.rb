...
class HeatStressWarningEnEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    seen = base.HeatStressWarningEn(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.HeatStressWarningEn(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_EN_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def heat_stress_warning_en_basic_setup(extra)
  ...
  entid_env_raw = ENV["HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_EN_ENTID"]
