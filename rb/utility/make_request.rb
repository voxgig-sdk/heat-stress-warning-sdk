...
module HeatStressWarningUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HeatStressWarningResponse.new({})
    ...
    if fetch_err
      ...
      response = HeatStressWarningResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HeatStressWarningResponse.new(fetched)
    else
