...
class HeatStressWarningSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = HeatStressWarningHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = HeatStressWarningHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, HeatStressWarningFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    HeatStressWarningUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = HeatStressWarningHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = HeatStressWarningHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => HeatStressWarningError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = HeatStressWarningHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = HeatStressWarningError.new(
        "graphql_error", "HeatStressWarningSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = HeatStressWarningSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
