...
class HeatStressWarningEnEntity
  ...
  def make
    opts = @_entopts.dup
    HeatStressWarningEnEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HeatStressWarningHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [HeatStressWarningEn, Hash] the current HeatStressWarningEn data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HeatStressWarningHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of HeatStressWarningEn fields)
  def match_get
  ...
  #
  # @param reqmatch [HeatStressWarningEnListMatch, Hash, nil] match filter (any subset of
  #   HeatStressWarningEn fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<HeatStressWarningEn>, Array] the matching HeatStressWarningEn items; raises HeatStressWarningError on failure
