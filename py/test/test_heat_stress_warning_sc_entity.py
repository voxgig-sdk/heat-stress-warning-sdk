...
class TestHeatStressWarningScEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.HeatStressWarningSc(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.HeatStressWarningSc(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID JSON to run live")
        client = setup["client"]
        ...
        heat_stress_warning_sc_ref01_list_result = heat_stress_warning_sc_ref01_ent.list(heat_stress_warning_sc_ref01_match, None)
...
def _heat_stress_warning_sc_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID"))
