# HeatStressWarning SDK test runner
...
class HeatStressWarningTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HeatStressWarningTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HeatStressWarningTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HeatStressWarningTestRunner.getenv("HEAT_STRESS_WARNING_TEST_LIVE")
        override = HeatStressWarningTestRunner.getenv("HEAT_STRESS_WARNING_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HeatStressWarningTestRunner.getenv(key)
        ...
        explain = HeatStressWarningTestRunner.getenv("HEAT_STRESS_WARNING_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if HeatStressWarningTestRunner._test_control is not None:
            return HeatStressWarningTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HeatStressWarningTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HeatStressWarningTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HeatStressWarningTestRunner.load_test_control()
...
def load_env_local():
    HeatStressWarningTestRunner.load_env_local()


def env_override(m):
    return HeatStressWarningTestRunner.env_override(m)


def entity_data(v):
    return HeatStressWarningTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HeatStressWarningTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HeatStressWarningTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HeatStressWarningTestRunner.load_test_control()


def live_client_options():
    return HeatStressWarningTestRunner.live_client_options()


def live_delay_ms():
    return HeatStressWarningTestRunner.live_delay_ms()
