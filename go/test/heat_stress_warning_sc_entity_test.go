...
func TestHeatStressWarningScEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.HeatStressWarningSc(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.HeatStressWarningSc(nil).Stream("list", nil, nil) {
...
func heat_stress_warning_scBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID"])
