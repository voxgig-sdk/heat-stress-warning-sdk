...
import (
	...
	omni "github.com/voxgig-sdk/heat-stress-warning-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HeatStressWarningError); is {
				out := map[string]any{
					"name":    "HeatStressWarningError",
...
func omniCtx(arg any, client *sdk.HeatStressWarningSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HeatStressWarningSDK); is {
			client = live
