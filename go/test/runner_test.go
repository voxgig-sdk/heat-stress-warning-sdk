...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("HEATSTRESSWARNING_TEST_LIVE") == "TRUE" ||
		os.Getenv("HEATSTRESSWARNING_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("HEATSTRESSWARNING_TEST_EXPLAIN"); explain != "" {
