...
func TestHeatStressWarningEnEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.HeatStressWarningEn(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.HeatStressWarningEn(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		heatStressWarningEnRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.heat_stress_warning_en")))
...
func heat_stress_warning_enBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_EN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_EN_ENTID"])
