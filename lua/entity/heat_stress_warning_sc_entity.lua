...
local HeatStressWarningScEntity = {}
HeatStressWarningScEntity.__index = HeatStressWarningScEntity


function HeatStressWarningScEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HeatStressWarningScEntity)
  self._name = "heat_stress_warning_sc"
  self._client = client
...
function HeatStressWarningScEntity:get_name()
  return self._name
end


function HeatStressWarningScEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HeatStressWarningScEntity.new(self._client, opts)
end
...
function HeatStressWarningScEntity:mark_deleted()
  self._deleted = true
end


function HeatStressWarningScEntity:deleted()
  return true == self._deleted
end


function HeatStressWarningScEntity:data_set(args)
  if args ~= nil then
...
function HeatStressWarningScEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HeatStressWarningScEntity:match_set(args)
  if args ~= nil then
...
function HeatStressWarningScEntity:match_get()
...
function HeatStressWarningScEntity:stream(action, args, callopts)
...
---@param reqmatch HeatStressWarningScListMatch
---@param ctrl? table
---@return HeatStressWarningSc[]
---@return string? err
function HeatStressWarningScEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function HeatStressWarningScEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return HeatStressWarningScEntity
