...
local HeatStressWarningEnEntity = {}
HeatStressWarningEnEntity.__index = HeatStressWarningEnEntity


function HeatStressWarningEnEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HeatStressWarningEnEntity)
  self._name = "heat_stress_warning_en"
  self._client = client
...
function HeatStressWarningEnEntity:get_name()
  return self._name
end


function HeatStressWarningEnEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HeatStressWarningEnEntity.new(self._client, opts)
end
...
function HeatStressWarningEnEntity:mark_deleted()
  self._deleted = true
end


function HeatStressWarningEnEntity:deleted()
  return true == self._deleted
end


function HeatStressWarningEnEntity:data_set(args)
  if args ~= nil then
...
function HeatStressWarningEnEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HeatStressWarningEnEntity:match_set(args)
  if args ~= nil then
...
function HeatStressWarningEnEntity:match_get()
...
function HeatStressWarningEnEntity:stream(action, args, callopts)
...
function HeatStressWarningEnEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return HeatStressWarningEnEntity
