...
local HeatStressWarningTcEntity = {}
HeatStressWarningTcEntity.__index = HeatStressWarningTcEntity


function HeatStressWarningTcEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, HeatStressWarningTcEntity)
  self._name = "heat_stress_warning_tc"
  self._client = client
...
function HeatStressWarningTcEntity:get_name()
  return self._name
end


function HeatStressWarningTcEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return HeatStressWarningTcEntity.new(self._client, opts)
end
...
function HeatStressWarningTcEntity:mark_deleted()
  self._deleted = true
end


function HeatStressWarningTcEntity:deleted()
  return true == self._deleted
end


function HeatStressWarningTcEntity:data_set(args)
  if args ~= nil then
...
function HeatStressWarningTcEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function HeatStressWarningTcEntity:match_set(args)
  if args ~= nil then
...
function HeatStressWarningTcEntity:match_get()
...
function HeatStressWarningTcEntity:stream(action, args, callopts)
...
function HeatStressWarningTcEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return HeatStressWarningTcEntity
