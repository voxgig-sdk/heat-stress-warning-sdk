# HeatStressWarning Lua SDK



The Lua SDK for the HeatStressWarning API — an entity-oriented client using Lua conventions.

It exposes the API as capitalised, semantic **Entities** — e.g. `client:HeatStressWarningEn()` — each with the same small set of operations (`list`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
GitHub release tag (`lua/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/heat-stress-warning-sdk/releases)),
...
| `HeatStressWarningEn` | `(data) -> HeatStressWarningEnEntity` | Create a HeatStressWarningEn entity instance. |
| `HeatStressWarningSc` | `(data) -> HeatStressWarningScEntity` | Create a HeatStressWarningSc entity instance. |
| `HeatStressWarningTc` | `(data) -> HeatStressWarningTcEntity` | Create a HeatStressWarningTc entity instance. |
...
| `recommendations` | `table` | List of recommended actions for workers |
...
```
lua/
├── heat-stress-warning_sdk.lua    -- Main SDK module
...
The main module (`heat-stress-warning_sdk`) exports the SDK constructor
...
heatstresswarningen:list()

-- heatstresswarningen:data_get() now returns the heatstresswarningen data from the last list
-- heatstresswarningen:match_get() returns the last match criteria
```
