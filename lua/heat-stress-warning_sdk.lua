...
require("heat-stress-warning_types")
...
local HeatStressWarningSDK = {}
HeatStressWarningSDK.__index = HeatStressWarningSDK
...
HeatStressWarningSDK._make_feature = _make_feature


function HeatStressWarningSDK.new(options)
  local self = setmetatable({}, HeatStressWarningSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function HeatStressWarningSDK:options_map()
...
function HeatStressWarningSDK:get_utility()
  return Utility.copy(self._utility)
end


function HeatStressWarningSDK:get_root_ctx()
  return self._rootctx
end


function HeatStressWarningSDK:prepare(fetchargs)
...
function HeatStressWarningSDK:direct(fetchargs)
...
function HeatStressWarningSDK:_op_allowed(op)
  local allow = vs.getpath(self.options, "allow.op")
  return type(allow) == "string" and allow:find(op, 1, true) ~= nil
end


function HeatStressWarningSDK:_op_denied(op)
  local allow = vs.getpath(self.options, "allow.op")
  if type(allow) ~= "string" then allow = "" end
  return {
    ok = false,
    err = "HeatStressWarningSDK: " .. op .. ": operation not allowed by" ..
...
-- by passing it.
function HeatStressWarningSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function HeatStressWarningSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "HeatStressWarningSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function HeatStressWarningSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = HeatStressWarningSDK.new(sdkopts)
  sdk.mode = "test"
