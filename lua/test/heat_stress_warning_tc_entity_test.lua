...
describe("HeatStressWarningTcEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:HeatStressWarningTc(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:HeatStressWarningTc(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_TC_ENTID JSON to run live")
      return
    end
...
function heat_stress_warning_tc_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_TC_ENTID")
