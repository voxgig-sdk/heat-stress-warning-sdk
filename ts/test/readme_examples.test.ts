...
//     (\`new HeatStressWarningSDK(...)\` / \`HeatStressWarningSDK.test(...)\` become
//     \`HeatStressWarningSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HeatStressWarningSDK'
...
const TEST_SEED = {"entity":{"heat_stress_warning_en":{"test01":{"id":"test01"}},"heat_stress_warning_sc":{"test01":{"id":"test01"}},"heat_stress_warning_tc":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HeatStressWarningSDK(...)\` and any
// \`HeatStressWarningSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HeatStressWarningSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HeatStressWarningSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
