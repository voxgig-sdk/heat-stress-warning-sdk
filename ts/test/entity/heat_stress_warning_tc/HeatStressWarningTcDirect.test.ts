...
describe('HeatStressWarningTcDirect', async () => {
  ...
  test('direct-list-heat_stress_warning_tc', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-heat_stress_warning_tc', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_TC_ENTID']
