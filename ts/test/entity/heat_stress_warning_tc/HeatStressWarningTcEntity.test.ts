...
describe('HeatStressWarningTcEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'heat_stress_warning_tc.' + op, live)) return
    }
    ...
    let heat_stress_warning_tc_ref01_data = Object.values(setup.data.existing.heat_stress_warning_tc)[0] as any

    // LIST
    const heat_stress_warning_tc_ref01_ent = client.HeatStressWarningTc()
    const heat_stress_warning_tc_ref01_match: any = {}

    const heat_stress_warning_tc_ref01_list = (await heat_stress_warning_tc_ref01_ent.list(heat_stress_warning_tc_ref01_match)).map((e: any) => e.data())
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/heat_stress_warning_tc/HeatStressWarningTcTestData.json')
  ...
  const idmapEnvVal = process.env['HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_TC_ENTID']
