...
describe('HeatStressWarningScDirect', async () => {
  ...
  test('direct-list-heat_stress_warning_sc', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-heat_stress_warning_sc', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HEAT_STRESS_WARNING_TEST_HEAT_STRESS_WARNING_SC_ENTID']
