...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.HEATSTRESSWARNING_TEST_LIVE ||
    'TRUE' === process.env.HEATSTRESSWARNING_TEST_OVERRIDE
  ) {
  ...
  m.HEATSTRESSWARNING_TEST_EXPLAIN = process.env.HEATSTRESSWARNING_TEST_EXPLAIN || m.HEATSTRESSWARNING_TEST_EXPLAIN
