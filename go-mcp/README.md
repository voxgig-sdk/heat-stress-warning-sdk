# heat-stress-warning-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HeatStressWarning SDK as
two agent tools — `heat-stress-warning_list` and `heat-stress-warning_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/heat-stress-warning-mcp)
make build

# 2. Provide credentials via the environment
export HEAT_STRESS_WARNING_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user heat-stress-warning \
  -- /absolute/path/to/heat-stress-warning-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./heat-stress-warning-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// heat-stress-warning_list: first page of records
{ "entity": "heat_stress_warning_en" }
{ "entity": "heat_stress_warning_en", "query": { } }

// heat-stress-warning_load: one record by id
{ "entity": "heat_stress_warning_en", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/heat-stress-warning-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user heat-stress-warning \
     -- "$PWD"/dist/*/heat-stress-warning-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `heat-stress-warning_list` and `heat-stress-warning_load` tools now appear
   in new sessions. Ask the agent to *"list heat_stress_warning_en using heat-stress-warning"*
   and it calls `heat-stress-warning_list` with `{"entity":"heat_stress_warning_en"}`.
...
```sh
export HEAT_STRESS_WARNING_APIKEY=sk_live_xxx            # API key
export HEAT_STRESS_WARNING_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `heat-stress-warning_list` tool
...
### Call the `heat-stress-warning_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `heat-stress-warning_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `heat-stress-warning_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HEAT_STRESS_WARNING_APIKEY` | API key sent with every request. |
| `HEAT_STRESS_WARNING_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

heat_stress_warning_en | heat_stress_warning_sc | heat_stress_warning_tc

### Smoke test via HTTP (raw JSON-RPC)

```sh
./heat-stress-warning-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"heat-stress-warning_load","arguments":{"entity":"heat_stress_warning_en","query":{"id":1}}}}'
```
