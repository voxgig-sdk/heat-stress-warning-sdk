...
type Args struct {
	Entity string         `json:"entity" jsonschema:"heat_stress_warning_en | heat_stress_warning_sc | heat_stress_warning_tc"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.HeatStressWarningSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "heat-stress-warning_list",
		Description: "List records from HeatStressWarning. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "heat-stress-warning_load",
		Description: "Load a single record from HeatStressWarning. " +
...
func runOp(client *sdk.HeatStressWarningSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.HeatStressWarningSDK, name string) (sdk.HeatStressWarningEntity, error) {
	switch strings.ToLower(name) {
	case "heat_stress_warning_en":
		return client.HeatStressWarningEn(nil), nil
	case "heat_stress_warning_sc":
		return client.HeatStressWarningSc(nil), nil
	case "heat_stress_warning_tc":
		return client.HeatStressWarningTc(nil), nil
